...
parser.add_argument('-t', '--trial', type=int, required=True, default=1)
parser.add_argument('--model', type=str, required=True, default='NAOMI', help='NAOMI, SingleRes')
parser.add_argument('--task', type=str, required=True, default='basketball', help='basketball, billiard')
parser.add_argument('--y_dim', type=int, required=True, default=10)
parser.add_argument('--rnn_dim', type=int, required=True, default=300)
parser.add_argument('--dec1_dim', type=int, required=True, default=200)
parser.add_argument('--dec2_dim', type=int, required=True, default=200)
parser.add_argument('--dec4_dim', type=int, required=True, default=200)
parser.add_argument('--dec8_dim', type=int, required=True, default=200)
parser.add_argument('--dec16_dim', type=int, required=True, default=200)
parser.add_argument('--n_layers', type=int, required=False, default=2)
parser.add_argument('--seed', type=int, required=False, default=123)
parser.add_argument('--clip', type=int, required=True, default=10, help='gradient clipping')
parser.add_argument('--pre_start_lr', type=float, required=True, default=1e-3, help='pretrain starting learning rate')
...
parser.add_argument('--discrim_rnn_dim', type=int, required=True, default=128)
...
for e in range(pretrain_epochs):
    ...
    _, _, _, _, _, _, _, _, mod_stats, exp_stats = \
        collect_samples_interpolate(policy_net, test_data, weighted_train_data, use_gpu, e, args.task, name='pretrain_inter', draw=True,
                                    stats=True)
    ...
    win_pre_path_length = vis.line(X=np.array([epoch]),
                                   ...
                                   win=win_pre_path_length, update=update,
    ...
    win_pre_out_of_bound = vis.line(X=np.array([epoch]),
                                    ...
                                    win=win_pre_out_of_bound, update=update,
    ...
    win_pre_step_change = vis.line(X=np.array([epoch]),
                                   ...
                                   win=win_pre_step_change, update=update,
    ...
    win_pre_policy = vis.line(X=np.array([epoch]), Y=np.column_stack((np.array([test_loss]), np.array([train_loss]))),
                              win=win_pre_policy, update=update,
                              opts=dict(legend=['out-of-sample loss', 'in-sample loss'],
...
for i in range(args.pretrain_disc_iter):
    exp_states, exp_actions, w_exp_states, w_exp_actions, exp_seq, model_states_var, model_actions_var, model_seq, mod_stats, exp_stats = \
        collect_samples_interpolate(policy_net, train_data, weighted_train_data, use_gpu, i, args.task,
                                    name="pretraining", draw=False,
                                    stats=False)
    ...
    pre_mod_p, pre_exp_p = update_discrim(discrim_net, optimizer_discrim, discrim_criterion, exp_states,
                                          exp_actions, w_exp_states, w_exp_actions, model_states, model_actions, i,
                                          dis_times=3.0, use_gpu=use_gpu,
                                          train=True)
...
for i_iter in range(args.max_iter_num):
    ...
    exp_states, exp_actions, w_exp_states, w_exp_actions, exp_seq, model_states_var, model_actions_var, model_seq, mod_stats, exp_stats = \
        collect_samples_interpolate(policy_net, train_data, weighted_train_data, use_gpu, i_iter, args.task, draw=False,
                                    stats=False)
    model_states = model_states_var.data
    model_actions = model_actions_var.data
    ...
    if i_iter % args.draw_interval == 0:
        _, _, _, _, _, _, _, _, mod_stats, exp_stats = \
            collect_samples_interpolate(policy_net, test_data, test_data, use_gpu, i_iter, args.task, draw=True,
                                        stats=True)
    ...
    if i_iter > 3 and mod_p[-1] < 0.8:
        update_policy(policy_net, optimizer_policy, discrim_net, discrim_criterion, model_states_var, model_actions_var,
                      i_iter, use_gpu)
    t1 = time.time()
    ...
    if args.save_model_interval > 0 and i_iter % args.save_model_interval == 0:
        torch.save(policy_net.state_dict(),
                   save_path + 'model/policy_step' + str(args.highest) + '_training_' + str(i_iter) + '.pth')
        torch.save(discrim_net.state_dict(),
                   save_path + 'model/discrim_step' + str(args.highest) + '_training_' + str(i_iter) + '.pth')
